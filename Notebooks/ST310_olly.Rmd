...
path <- "/Users/Ollym/OneDrive/Documents/GitHub/ST310-Group-Project/Data/Stellar-Classification-Dataset.csv"
...
```{r}
#Calculates accuracy
accuracy(rf_red_pred_df, as.factor(class), as.factor(.pred_class))
```
...
```{r}
#Calculates accuracy
accuracy(rf_i_pred_df, as.factor(class), as.factor(.pred_class))
```
The lowest mis-classification rate we achieve is 2.4%, with the forest with no interaction terms. 
